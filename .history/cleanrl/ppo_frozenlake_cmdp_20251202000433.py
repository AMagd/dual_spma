...
def estimate_d_pi_from_obs_action(obs_buffer: torch.Tensor, action_buffer: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    """
    T, N, n_states, n_actions = obs_buffer.shape
    flat = obs_buffer.reshape(-1, n_states, n_actions)  # [(T*N), n_states, n_actions]
    d_pi = flat.mean(dim=0)  # average one-hot -> empirical distribution
    d_pi = d_pi / (d_pi.sum(dim=-1, keepdim=True) + eps)
    return d_pi
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            d_bar = d_bar * (1 - args.d_bar_learning_rate) + d_k * args.d_bar_learning_rate
